...
export function MoviesCardList({ onCardLike, onCardDelete, initialMovies }) {  // Передаются функции открытия попапов из App.js
  return ( // Отрисовываем каждый фильм при помощи компонента Movie и возвращаем в разметку внутрь section
    <section id="elements" className="elements page__elements-position section">
      {initialMovies && initialMovies.map(movie => (// Пробегаем по переданному массиву и возвращаем карточки с фильмами
